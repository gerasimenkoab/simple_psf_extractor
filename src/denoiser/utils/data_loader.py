...
__all__ = ['fluore_to_tensor', 'DenoisingFolder', 'DenoisingFolderN2N',
           ...
           'load_denoising_n2n_train', 'load_denoising_test_mix', 'find_latest_model']
...
def find_latest_model(_folder_path: str) -> Tuple[str, str, int]:
    ...
    if latest_model_path is None:
        raise ValueError(f"No model found in path: {_folder_path} ")
    latest_datetime = datetime.fromtimestamp(os.path.getctime(latest_model_path))
    formatted_datetime = latest_datetime.strftime("%d.%m.%Y-%H:%M:%S")
    print(f"Model path: {latest_model_path}\nCreation date: {formatted_datetime}\nModel version:{model_version}")
    return latest_model_path, formatted_datetime, model_version


if __name__ == '__main__':
    root = '../../Denoising/dataset'
    loader = pil_loader
    train = True
    noise_levels = [1, 2, 4]
    types = ['Confocal_MICE']
