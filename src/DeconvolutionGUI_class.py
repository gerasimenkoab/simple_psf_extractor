...
class DeconvolutionGUI(Toplevel):
    ...
    def LoadDeconvPhoto(self):
        ...
        fig.suptitle("Bead")
        axs[0].pcolormesh(
            self.img.imArray[self.img.imArray.shape[0] // 2, :, :], cmap=cm.jet
        )
        axs[1].pcolormesh(
            self.img.imArray[:, self.img.imArray.shape[1] // 2, :], cmap=cm.jet
        )
        axs[2].pcolormesh(
            self.img.imArray[:, :, self.img.imArray.shape[2] // 2], cmap=cm.jet
        )
        # plt.show()
        # Instead of plt.show creating Tkwidget from figure
        self.figIMG_canvas_agg = FigureCanvasTkAgg(fig, self.cnvImg)
