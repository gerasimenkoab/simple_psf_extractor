...
class DeconvolutionGUI(Tk):
    ...
    #    Button(f2,text = 'Save PSF multi-file',command=self.SavePSFMulti).grid(row=5, column=0)
        ...
        Separator(f3, orient="horizontal").grid( row=9, column=0, ipadx=200, padx=30,pady=10, columnspan=2 )
        Button(f3, text="Close", command=self.CloseWindow).grid(
            row=10, column=1
        )
    ...
    def DeconvolveIt(self):
        ...
        try:
            ...
            try:
                  ...
                  self.imgDecon = decon.richardson_lucy_deconvolution_3d_tv(self.img.imArray, self.imgPSF, iterLim, beta=0.5, tv_weight=0.1)
            except:
