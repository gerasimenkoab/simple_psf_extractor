...
class MainAppView(tk.Tk):
    def __init__(self, masterController = None,  wWidth = 300, wHeight = 250):
        ...
        b1 = ttk.Button(self, text="Bead Extractor ", command = lambda: self.event_generate("<<RunBeadExtractorWidget>>"))
        b1.pack(fill='x', side = 'top',padx=20,pady=10)
        b2 = ttk.Button(self, text="Deconvolutor ", command = lambda: self.event_generate("<<RunDeconvolutionWidget>>"))
        b2.pack(fill='x', side = 'top',padx=20,pady=10)
        b3 = ttk.Button( self, text = "NN Deconvolutor ", command = lambda: self.event_generate("<<RunNNDeconvolutionWidget>>"))
